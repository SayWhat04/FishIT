...
export class SupabaseService {
  ...
  getCurrentUser(): Observable<User | null> {
    return from(supabase.auth.getUser()).pipe(map(response => response.data?.user || null));
  }
  ...
  getById(tableName: TableName, id: string): Observable<any> {
    return from(supabase.from(tableName).select('*').eq('id', id).single()).pipe(
      map(response => {
        const { data, error } = response as any;
        if (error) {
          throw error;
  ...
  insert(tableName: TableName, data: any): Observable<any> {
    return from(supabase.from(tableName).insert(data).select().single()).pipe(
      map(response => {
  ...
  update(tableName: TableName, id: string, data: any): Observable<any> {
    return from(supabase.from(tableName).update(data).eq('id', id).select().single()).pipe(
      map(response => {
        const { data: result, error } = response as any;
        if (error) {
          throw error;
  ...
  delete(tableName: TableName, id: string): Observable<void> {
    return from(supabase.from(tableName).delete().eq('id', id)).pipe(
      map(response => {
