...
export class ReviewPageComponent implements OnInit {
  ...
  saveAcceptedFlashcards(): void {
    ...
    if (acceptedFlashcards.length === 0) {
      this.snackBar.open('No flashcards have been accepted. Please review and accept some flashcards.', 'OK', { duration: 5000 });
      return;
    }
    ...
    this.boxService.addFlashcardsToBox(this.selectedBoxId()!, acceptedFlashcards).subscribe({
      next: () => {
        ...
        this.snackBar.open(`${acceptedFlashcards.length} flashcards added to box.`, 'OK', { duration: 5000 });
        this.router.navigate(['/boxes', this.selectedBoxId()]);
      },
      error: () => {
        this.isLoading.set(false);
        this.snackBar.open('Failed to add flashcards to box. Please try again.', 'OK', { duration: 5000 });
      }
    });
  }
} 